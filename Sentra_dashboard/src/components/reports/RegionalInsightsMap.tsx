...
export const RegionalInsightsMap = () => {
  ...
  const handleTokenSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (tokenInput?.value) {
      ...
      localStorage.setItem("mapbox-token", tokenInput.value);
    }
  };
    ...
    <div className="h-full w-full relative bg-smartel-green-200/30 rounded-lg overflow-hidden border border-white/30">
      {!mapboxToken ? (
        <div className="absolute inset-0 flex flex-col items-center justify-center p-6">
          ...
          <p className="text-muted-foreground mb-4">Please enter your Mapbox access token to load the map</p>
          ...
          <form onSubmit={handleTokenSubmit} className="w-full max-w-md">
            <div className="flex gap-2">
              <input 
                ...
                className="flex h-10 w-full rounded-md border border-white/30 bg-white/20 px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
              />
              <button 
                type="submit"
                className="bg-smartel-green-500 hover:bg-smartel-green-600 text-white px-4 py-2 rounded text-sm"
              >
                Load Map
              </button>
            </div>
            <p className="text-xs text-muted-foreground mt-2">
              Get your token at <a href="https://mapbox.com" target="_blank" rel="noopener noreferrer" className="underline">mapbox.com</a> (free account required)
            </p>
          </form>
        </div>
      ) : (
